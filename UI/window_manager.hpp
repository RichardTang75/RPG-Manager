...
public:
    ...
    SDL_Rect get_spacing()
    {
        return SDL_Rect{x_offset, y_offset, x_increment, y_increment};
    }
    ...
    {
        background_img->draw_this_slice(to_use, background);
        draw_outline(to_use);
    }
    ...
    void title_drawer(std::string title, SDL_Renderer* to_use, TTF_Font* font_use, SDL_Color what_color, int down, int x_off=0)
    {
        ...
        int middle=(width)/2+start_x+x_off;
        write_text_centered(middle, start_y+down, title, to_use, font_use, what_color);
    }
    bool shown = false;
...
public:
    ...
    virtual void draw_full(SDL_Renderer* to_use, TTF_Font* font_use, SDL_Color what_color)
    {
        ...
        std::vector<int> changed_hero_indices;
        for (int i=0; i<every_hero.size(); i++)
        {
            hero current_hero = every_hero[i];
            if (current_hero.changed)
            {
                current_hero.changed=false;
                changed=true;
                changed_hero_indices.push_back(i);
            }
        }
        if (changed==true && sub_rects.size()==every_hero.size())
        {
            changed = false;
            for (int & index : changed_hero_indices)
            {
                hero current_hero = every_hero[index];
                std::string text = "    " + current_hero.name + "\n        Archetype: " + current_hero.get_archetype().name + "    Level: " +
                std::to_string(current_hero.level) +  "     Health: " +std::to_string(int(current_hero.hp)) + "     Damage: " + std::to_string(int(current_hero.damage)) + "    Currently: " + current_hero.current_activity;
                button new_button = button(button_img, button_x_start, button_y_start + index * y_increment, button_width, sub_height, text, use::sub_but, 0, 0);
                sub_rects[index] = new_button;
            }
        }
        ...
        {
            sub.draw(to_use, font_use, what_color, 0, 0, 0);
...
{
public:
    hero_infoscreen (texture_holder* background, texture_holder* button_img, int x1, int y1, int x2, int y2, int z, int x_off, int y_off, int x_inc,
                     int y_inc, std::vector<hero>* every_hero_ref, int sub_x, int sub_y,
...
public:
    ...
    virtual std::pair<subwindow_reason, int> handle_click(int mouse_x, int mouse_y)
    {
        std::vector<int> possible_hero_indices = get_valid_hero_indices();
        if (mouse_x>button_x_start && mouse_x<button_x_start + sub_width && mouse_y>button_y_start)
        {
            int clicked_sub_rect = (mouse_y - button_y_start)/y_increment;
            if (clicked_sub_rect < sub_rects.size()-1) //-1 is for the submit button
            {
                auto inside_selected_iterator = std::find(selected_hero_indices.begin(), selected_hero_indices.end(),
                                                          possible_hero_indices[clicked_sub_rect]);
                if (inside_selected_iterator != selected_hero_indices.end())
                {
                    selected_hero_indices.erase(inside_selected_iterator);
                }
                else
                {
                    selected_hero_indices.push_back(possible_hero_indices[clicked_sub_rect]);
                }
            }
        }
        if (sub_rects.back().in_bounds(mouse_x, mouse_y))
        {
            return std::make_pair(subwindow_reason::none, true);
        }
        return std::make_pair(subwindow_reason::none,-1);
    }
    std::vector<int> get_selected()
    {
        std::vector<int> to_return = selected_hero_indices;
        selected_hero_indices.clear();
        return to_return;
    }
    virtual void draw_full(SDL_Renderer* to_use, TTF_Font* font_use, SDL_Color what_color)
    {
        ...
        draw_outline(to_use);
        std::vector<hero> every_hero = *hero_ref;
        std::vector<int> available = get_valid_hero_indices();
        if (changed==true)
        {
            changed = false;
        ...
        else if (sub_rects.size()==(available.size()+1))
        {
        }
        else
        {
            ...
            {
                hero current_hero = every_hero[available[i]];
            ...
            button submit_button = button(button_img, button_x_start+sub_width/4, end_y-y_increment*2, sub_width/2, y_increment, " ", use::sub_but);
            sub_rects.push_back(submit_button);
        }
        for (int i=0; i<sub_rects.size()-1; i++) //-1 prevents last one from being counted, submit button
        {
            button sub = sub_rects[i];
            if (std::find(selected_hero_indices.begin(), selected_hero_indices.end(), available[i]) != selected_hero_indices.end())
            {
                sub.draw(to_use, font_use, what_color, 255, 255, 0);
            }
            else
            {
                sub.draw(to_use, font_use, what_color, 0, 0, 0);
        ...
        //draws last one here
        sub_rects.back().draw(to_use, font_use, what_color, 0, 0, 0);
        SDL_SetRenderDrawColor(to_use, 200, 200, 200, 255);
        SDL_RenderDrawLine(to_use, start_x+x_offset, start_y+y_offset, end_x-x_offset, start_y+y_offset);
    }
    ...
    window_UI* currently_attached_to;
private:
    std::vector<int> get_valid_hero_indices()
    {
        std::vector<int> available;
        std::vector<hero> every_hero = *hero_ref;
        for (int i=0; i<every_hero.size(); i++)
        {
            hero possible = every_hero[i];
            if (possible.current_activity == "Resting")
            {
...
public:
                      ...
                      int sub_e_x, int sub_h, std::unordered_map<std::string, texture_holder>& archetype_icons): window_UI (background, button_img, x1, y1, x2, y2, z, x_off, y_off, x_inc, y_inc)
    {
        ...
        sub_end_x_offset = sub_e_x;
        sub_height = sub_h;
        hero_ref = every_hero_ref; //is this needed
